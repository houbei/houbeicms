...
<div id="article-list">
            ...
            //---------------------------查询条件-------------------------------
                ...
                if($cid){
                    ...
                    if($category['cat_url_type']==2){
                        ...
                        Page::$staticUrl='http://127.0.0.1/'.$html_path.
    ...
    <div class="menu">
        <div class="send">
            <a href="http://127.0.0.1/index.php?m=Member&c=Content&a=add&mid=<?php echo $hd['get']['mid'];?>&cid=<?php echo $hd['get']['cid'];?>">发表文章</a>
        </div>
        <dl>
            <dt>文章分类</dt>
                    <?php
        ...
        if($result){
            ...
            foreach ($result as $index=>$field):
                ...
                $field['_first']=$index==0?true:false;
                $field['_last']=$index==(count($result)-1)?true:false;
                $field['class']=$currentCid==$field['cid']?"active":'';
                $field['caturl'] = Url::category($field);
                if($field['cattype']==3){
                    $field['catlink']='<a href="'.$field['caturl'].'" target="_blank">'.$field['catname'].'</a>';
                }else{
